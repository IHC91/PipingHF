...
class frameItObserver(frameObserverPrototype):
    def __init__(self):
        super(frameItObserver, self).__init__(
            translate("fObservers", "Select one beam and one edge")
        )
        self.beam = self.edge = None
...
class fillFrameObserver(frameObserverPrototype):
    def __init__(self):
        super(fillFrameObserver, self).__init__(
            translate("fObservers", "First select the base beam, then the edges")
        )
        self.beam = None
    ...
    def addSelection(self, doc, obj, sub, pnt):
        if (
            self.beam == None
            and FreeCAD.getDocument(doc).getObject(obj).TypeId == "Part::FeaturePython"
        ):
        ...
        if subObject.ShapeType == "Face":
            if self.targetFace == None:
                ...
                FreeCAD.activeDocument().openTransaction(
                    translate("fObservers", "levelTheBeam", "Transaction")
                )
...
class alignFlangeObserver(frameObserverPrototype):
    def __init__(self):
        super(alignFlangeObserver, self).__init__(
            translate("fObservers", "Select the target face, then the others")
        )
        self.faceBase = None
...
class alignEdgeObserver(frameObserverPrototype):
    def __init__(self):
        super(alignEdgeObserver, self).__init__(
            translate("fObservers", "Select two edges to join.")
        )
        self.edges = []
    ...
    def addSelection(self, doc, obj, sub, pnt):
        ...
        if len(self.edges) > 1:
            ...
            beam = sel[len(sel) - 1]
            FreeCAD.activeDocument().openTransaction(
                translate("fObservers", "joinTheBeamsEdges", "Transaction")
            )
...
class stretchBeamObserver(frameObserverPrototype):  # OBSOLETE: replaced with dialog
    def __init__(self):
        super(stretchBeamObserver, self).__init__(
            translate("fObservers", "Select the beam and input the length")
        )
        self.beam = None
        ...
        if self.beam == None and Obj.TypeId == "Part::FeaturePython" and hasattr(Obj, "Height"):
            ...
            dist = float(
                qid.getText(
                    None,
                    "stretch a beam",
                    "old length = " + str(self.beam.Height) + "\nnew length?",
                )[0]
            )
...
class extendObserver(frameObserverPrototype):  # OBSOLETE: replaced with dialog
    ...
    def addSelection(self, doc, obj, sub, pnt):
        ...
        if (
            lastSel.TypeId == "Part::FeaturePython"
            and hasattr(lastSel, "Height")
...
class adjustAngleObserver(frameObserverPrototype):
    ...
    def addSelection(self, doc, obj, sub, pnt):
        ...
        if (
            lastSel.TypeId == "Part::FeaturePython"
            and hasattr(lastSel, "Height")
            and subLastSel.ShapeType == "Edge"
        ):
            self.edges.append(subLastSel)
            self.beams.append(lastSel)
            FreeCAD.Console.PrintMessage(
                "Edge/beam pair nr." + str(len(self.edges)) + "  selected.\n"
            )
...
class rotjoinObserver(frameObserverPrototype):  # OBSOLETE
    def __init__(self):
        super(rotjoinObserver, self).__init__(
            translate("fObservers", "Select 2 edges =>[Ctrl]+select")
        )
        self.edges = []
    ...
    def addSelection(self, doc, obj, sub, pnt):
        ...
        if len(self.edges) == 2:
            try:
                FreeCAD.activeDocument().openTransaction(
                    translate("fObservers", "rotJoin", "Transaction")
                )
                fCmd.rotjoinTheBeam()
                FreeCAD.activeDocument().commitTransaction()
                FreeCAD.activeDocument().recompute()
