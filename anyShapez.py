...
class AnyThing(pypeType):  # object):
    ...
    def __init__(self, obj, name="valve", fileName="ballDN15.stp", ports="0:0:0"):
        ...
        obj.addProperty(
            "App::PropertyString", "FileName", "AnyThing", "The file of the shape (inside ./shapez)"
        ).FileName = fileName
        portslist = list()
        if ports:
...
class shapezDialog(dodoDialogs.protoTypeDialog):
    ...
    def __init__(self):
        ...
        for n in listdir(self.cwd):
            ...
            if isdir(absFile):
                self.form.comboDirs.addItem(n)
            elif isfile(absFile) and n.upper().split(extsep)[-1] in [
                "STEP",
                "STP",
                "IGES",
                "IGS",
    ...
    def accept(self):
        ...
        if self.form.comboDirs.currentText() == "<shapes>":
            ...
            sdir = self.form.comboDirs.currentText()
    ...
    def fillFiles(self):
        ...
        if self.form.comboDirs.currentText() != "<shapes>":
            ...
            self.cwd = self.shapesDir
        for n in listdir(self.cwd):
            if isfile(join(self.cwd, n)) and n.upper().split(extsep)[-1] in [
                "STEP",
                "STP",
                "IGES",
                "IGS",
